...
BEGIN
    ...
    SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) INTO v_result FROM (
        ...
        FROM tab_Orden o
        JOIN tab_Usuarios u ON o.id_usuario = u.id_usuario -- FK NOT NULL: toda orden tiene comprador
        -- LATERAL: Obtiene el pago más reciente, evitando duplicados si hay múltiples registros.
        JOIN LATERAL (
            SELECT pg.comprobante_ruta, pg.estado_pago
            FROM tab_Pagos pg
            WHERE pg.id_orden = o.id_orden
            ORDER BY pg.id_pago DESC
            LIMIT 1
        ) p ON TRUE
        JOIN tab_Envios e ON o.id_orden = e.id_orden -- UNIQUE(id_orden): relación 1:1 garantizada
        JOIN tab_Direcciones_Envio de ON e.id_direccion_envio = de.id_direccion -- FK NOT NULL
        JOIN tab_Ciudades ci ON de.id_ciudad = ci.id_ciudad -- FK NOT NULL
        JOIN tab_Departamentos dep ON ci.id_departamento = dep.id_departamento -- FK NOT NULL
        WHERE
...
BEGIN
    ...
    SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) INTO v_result FROM (
        ...
        FROM tab_Reservas r
        JOIN tab_Servicios s ON r.id_servicio = s.id_servicio -- FK NOT NULL: toda reserva tiene servicio
        JOIN tab_Usuarios u ON r.id_usuario = u.id_usuario    -- FK NOT NULL: toda reserva tiene cliente
