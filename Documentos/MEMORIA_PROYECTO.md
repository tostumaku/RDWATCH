...
- [x] **Panel Administrativo UI**: Layout responsive con `cats-layout`, botonera unificada y corrección del ancho de la columna "Acciones" compacta. Se removieron spinners numéricos en inputs de inventario para un diseño más limpio.
- [x] **PostgreSQL Soft Delete**: Se actualizó la función `fn_cat_delete_producto` reduciendo de 2 a 1 parámetro para alinearlo con el Request del backend en PHP, habilitando correctamente las desactivaciones lógicas recuperables.
- [x] **Facturación & Checkout**: Se solucionó error de suma en `factura.js` agregando el costo de envío. Se implementaron override en CSS para anular la herencia oscura de `.contact-form`, mejorando drásticamente legibilidad de los labels en fondo blanco.
- [x] **Panel de Usuario**: Implementador nativo del Frontend para conversión de fechas ISO 8601 de la BD a formato limpio local (`DD/MM/YYYY HH:MM`) eliminando métricas de microsegundos en el historial de pedidos.
- [ ] Probar flujo completo en el navegador (carrito → checkout → reseña → servicio → ciudades → eliminar items desde admin).

---
*Última edición técnica: Lunes 20 Abril 2026 — Mejoras UI Admin/Checkout, Facturación y Panel de Usuario*
